...
const imageGeneneration = async (initialPrompt) => {
	...
	try {
		...
		const detailed8BitPreface =
			'"Generate an image, the artwork should reflect scientific themes, incorporating elements such as schematics, equations, or laboratory setups in a visually striking format."';

		let fullPrompt = `${detailed8BitPreface} ${prompt} Remember, this is in striking scientific style`;
